...
export default function playQuizDuel(){
    ...
    useEffect(() => {
        ...
        socket.on("player-finished", (data) => {
            if (data.player == user?.name){
                showToast("Você terminou o quiz!", "info", 4000);
            } else {
                showToast(`${data.player} terminou o quiz!`, "info", 3500);
            }
            setTimeout(() => {
    ...
    if (question != null) 
        ...
        <div className="w-full text-center items-center justify-center flex flex-col pb-10 text-(--foreground) pl-2 pr-2">
            <div className="flex w-full h-fill flex-row justify-between items-center bg-(--area-back) rounded-lg overflow-auto gap-10 py-2">
                <h2 className="font-bold md:text-2xl text-sm pl-2 text-left">Quiz: {quizName}</h2>
                {
                <div className="w-fill px-4 h-full text-xs md:text-lg">
                    <h2 className="font-bold pr-2 text-right md:text-lg py-2">Pontuação:</h2>
            ...
            <div className="flex flex-col mt-4 w-full justify-between">
                <div className="flex flex-row md:gap-10 gap-4 font-bold w-full">
                    ...
                    <div className="w-full">
                        ...
                        <h2 className="text-left">{enemyName}</h2>
                            <h2 className="hidden md:block">{progressPlayer2}%</h2>
                            <div className="h-fill bg-blue-200 md:w-80 w-full rounded-lg">
                                <div className="bg-green-500 h-3 md:h-6 rounded-lg transition-all duration-500" style={{ width: `${progressPlayer2}%` }}></div>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div className="bg-(--area-back) rounded-lg py-4 w-full h-fill mt-10 flex flex-col gap-3 md:gap-6 text-center items-center justify-center">
                ...
                question?.possible_answers.map((answer, index) => (
                    <div key={index} onClick={() => sendAnswer(index)} className="md:h-13 h-fill py-3 md:w-200 w-full bg-blue-200 text-black items-center text-center justify-center flex rounded-xl hover:bg-blue-400 duration-300 cursor-pointer">
